return function()
    ...
    -- Creates a new NetworkHandlerObject with the given ID and wire type.
    function NetworkHandlerObject.NewNetwork(networkID, wireType)
        local network = {
            WireType = wireType,
            NetworkID = networkID,
            ControllerNodes = {},
    ...
    function NetworkHandlerObject:AddNode(node, fireNodeEvents)
    ...
    function NetworkHandlerObject:RemoveNode(node, fireNodeEvents)
        -- Get the nodes handler
        local nodeHandler = SE.NodeHandlersRegistry.GetNodeHandler(node)
        local nodeType = nodeHandler.Type
    ...
    function NetworkHandlerObject:OnTick(tick)
        -- Draw idle power
        self.HasPower = (self.PowerDrainNodeCount == 0) or NetworkHandlerObject.ExtractPower(self, self.PowerDrainNodeCount * SE.Settings.NodeIdlePowerDrain)
    ...
    function NetworkHandlerObject:NetworkTick()
    ...
    local function CanExtractPower(network, request)
        ...
        for controller, handler in pairs(network.ControllerNodes) do
            ...
            if (bridgedNetworkID ~= nil and request.VisititedNetworks[bridgedNetworkID] == nil) then
                ...
                if (CanExtractPower(SE.NetworksManager.GetNetwork(bridgedNetworkID), request) == true) then
                    -- Request fully satisfied
                    return true
                end
            end
    ...
    function NetworkHandlerObject:ExtractPower(watts)
        ...
        for node, handler in fpairs(network.StorageNodes, filterFn) do
            ...
            if (stackTransfering.count > 0) then
                ...
                while (stackTransfering.count > 0) do
                    ...
                    if (NetworkHandlerObject.ExtractPower(network, itemPower + chunkPower)) then
    ...
    function NetworkHandlerObject:InsertItems(stack, requesterNode)
    ...
    function NetworkHandlerObject:ExtractItems(stack, requesterNode)
    ...
    function NetworkHandlerObject:GetStorageContents(tick)
        -- New tick?
    ...
    function NetworkHandlerObject:Empty()
        return next(self.ControllerNodes) == nil and NetworkHandlerObject.EmptyEmptyExceptControllers(self)
    end

    -- EmptyExceptControllers( Self ) :: bool
    -- Returns true if there are no nodes, except controllers, on the network
    function NetworkHandlerObject:EmptyEmptyExceptControllers()
        return next(self.PowerSourceNodes) == nil and next(self.StorageNodes) == nil and next(self.DeviceNodes) == nil
    end

    return NetworkHandlerObject
end
